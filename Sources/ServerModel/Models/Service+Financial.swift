...
extension Service {
    public final class Financial: Model, Content {
        public static let schema = "serviceFinancials"
        ...
        // MARK: Parent
        @Parent(key: "serviceID")                                                       public var service:                Service
        
        // MARK: Children
        @Children(for: \.$financial)                                                    public var events:                 [KeystoneEvent]

        public init() {}
        
        public init(_ title: String, info: String?, total: String, subtotal: String, discount: String, tax: String, collected: Bool, percentagePaid: Int, stripeID: String?, for serviceID: Service.IDValue) {
            self.title = title
            self.info = info
            self.total = total
        ...
        convenience public init(data: ObjectData) {
            self.init(data.title, info: data.info, total: data.total, subtotal: data.subtotal, discount: data.discount, tax: data.tax, collected: data.collected, percentagePaid: data.percentagePaid, stripeID: data.stripedID, for: data.serviceID)
        }
    }
}

extension Service.Financial {
    public struct ObjectData: Content {
        public let collected:               Bool
        public let createdAt:               Date?
        public let discount:                String
        public let events:                  [KeystoneEvent.ObjectData]?
        public let id:                      Service.Financial.IDValue?
    ...
    public func objectData() throws -> ObjectData {
        return try ObjectData(collected: self.collected, createdAt: self.createdAt, discount: self.discount, events: nil, id: self.requireID(), info: self.info, percentagePaid: self.percentagePaid, serviceID: self.$service.id, stripedID: self.stripeID, subtotal: self.subtotal, tax: self.tax, title: self.title, total: self.total, updatedAt: self.updatedAt)
    }
}

extension Sequence where Element: Service.Financial {
